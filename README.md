...
**Интерфейс TUI 2.0.1.** Интерфейс по-русски по умолчанию; `/lang ru|en` (или
...
LLM → F9 сохранённые файлы / F2 ответ); `?` открывает справку со всеми клавишами и
...
**Очередь и ввод.** Перетаскивание или вставка добавляет файлы в рамку очереди
без Enter; папки обходятся рекурсивно в фоне, неподдерживаемые файлы пропускаются.
Повторный путь выделяет уже добавленный файл и не сбрасывает его результат.
«Добавить» / Insert / `/add` открывает подписанное поле ручного ввода;
стрелки, Home/End, Backspace/Delete и Ctrl+U редактируют текст, Esc закрывает поле.
Вне редактирования Delete / × удаляет одну запись, «Вернуть» / Ctrl+Z / `/undo`
восстанавливает последнее удаление. «Полный путь» раскрывает путь и сведения
выбранного файла. На узком экране питомец скрывается, освобождая место очереди.

`s` / F5 запускает только ожидающие файлы. Enter на записи / «Действия»
позволяет повторить ошибки (`/retry`) или обработать выбранный файл
(`/run-selected`); повтор готового результата требует подтверждения.
Запуск блокируется сразу, до ответа worker. «Очистить» / `/clear` отменяет
незавершённое добавление и очищает только очередь — готовые файлы и результаты
сессии сохраняются. Ошибки добавления видны в журнале; `/retry-input` повторяет
последний неудачный блок, сохранённый при новом вводе или потере связи.
Буквенные сочетания имеют русские алиасы; F1–F12 работают независимо от раскладки.

**Связь и остановка.** При запуске TUI проверяет совместимость worker и ждёт
готовности движка, не блокируя интерфейс. При потере связи — Ctrl+R,
`/reconnect` или «Переподключить»: очередь и результаты сохраняются,
предыдущее задание не запускается повторно. Переподключение разрешено только
после подтверждённого завершения старого worker.
Во время ASR Esc / «После файла» завершает текущий файл и останавливает очередь;
во время LLM Esc / «Отменить запрос» запрашивает отмену. Ещё Esc / «Прервать сейчас»
открывает подтверждение: Yes завершает worker с его дочерними процессами, No/Esc
возвращает к ожиданию мягкой остановки. Незавершённый вывод может потеряться;
удалённый API способен продолжить обработку на своей стороне.
Большие ответы не обрезаются как журнал: TUI принимает JSON-сообщения до 8 МиБ;
превышение даёт ошибку связи, а не бесконечное ожидание. Для ASR worker передаёт
TUI только статусы и пути результатов, сами транскрипты сохраняются в файлах.

**Прогресс и результаты.** «Всего» относится к партии, ниже — процент текущего
файла; подготовка без оценки обозначается «—». Итог показывает готовые,
ошибочные, не начатые и прерванные файлы и время работы. F9 / «Результаты»
(также `r`/`к`, когда есть результаты) открывает список сохранённых файлов сессии
даже после очистки очереди. Стрелки выбирают файл, PgUp/PgDn прокручивают полный
путь, Enter открывает текстовый результат системным просмотрщиком, O — папку,
Esc закрывает список. Ничего не открывается автоматически; пропавший файл
показывает ошибку. Сейчас список хранит результаты ASR и последнего запуска LLM;
история прежних LLM-запусков ещё не добавлена. Восстановление терминала проверено на macOS при обычном выходе,
сбое worker и раскрутке стека после panic; Windows пока проверен только компиляцией.
...
`/llm-provider-name`, `/llm-args`, `/llm-tools on|off`; F2 открывает вкладку LLM
с ответами, F9 — сохранённые файлы. Первый Esc во время LLM запрашивает отмену,
не завершая worker; повторный Esc предлагает подтверждённое принудительное завершение.
