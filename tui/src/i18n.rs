...
pub(crate) const STRINGS: &[(&str, &str, &str)] = &[
    ...
    ("status.worker_connecting", "Подключаем движок…", "Connecting worker…"),
    ("status.worker_timeout", "Движок не ответил за 15 секунд. Можно переподключить.", "Worker did not respond within 15 seconds. Reconnect to retry."),
    ("status.worker_incompatible", "Несовместимый протокол: {error}. Обновите TUI и worker вместе.", "Incompatible protocol: {error}. Update TUI and worker together."),
    ("status.reconnecting", "Останавливаем прежний движок перед подключением…", "Stopping previous worker before reconnecting…"),
    ("status.stop_failed", "Остановка не подтверждена: {error}", "Termination could not be confirmed: {error}"),
    ("status.force_stopping", "Прерываем обработку…", "Terminating processing…"),
    ("status.llm_starting_short", "Запускаем LLM…", "Starting LLM…"),
    ("hint.worker_connecting", "Ожидаем готовности движка; ввод и настройки доступны", "Waiting for worker; input and settings remain available"),
    ("button.reconnect", "Переподключить", "Reconnect"),
    ("button.force_stop", "Прервать сейчас", "Terminate now"),
    ("cmd.reconnect", "переподключить движок без повторного запуска задач", "reconnect the worker without repeating jobs"),
    ("confirm.force_stop", "Прервать сейчас? Незавершённый результат может быть потерян. Сохранённые файлы останутся. Запрос к внешнему API может продолжиться на сервере.", "Terminate now? Unfinished output may be lost. Saved files remain. An external API request may continue on the server."),
    ("status.adding_inputs", "Добавляем файлы…", "Adding files…"),
    ("status.inputs_cancelled", "Добавление отменено", "Adding cancelled"),
    ("input.error_missing", "Не найден", "Not found"),
    ("input.error_unreadable", "Нет доступа", "Cannot read"),
    ("input.error_unsupported", "Неподдерживаемый формат", "Unsupported format"),
    ("input.error_empty", "В папке нет медиафайлов", "No media files in folder"),
    ("input.error_invalid", "Некорректный путь", "Invalid path"),
    ("input.error_unknown", "Ошибка добавления", "Cannot add input"),
    ("status.batch_starting", "Запускаем обработку…", "Starting processing…"),
    ("status.confirm_rerun", "Повторить обработку выбранного файла? Результаты могут быть перезаписаны.", "Reprocess the selected file? Existing results may be overwritten."),
    ("status.invalid_input_response", "Некорректный ответ при добавлении файлов — ввод сохранён", "Invalid input response — input preserved"),
    ("status.resolver_unavailable", "Worker не поддерживает добавление файлов. Обновите TUI и worker вместе; ввод сохранён.", "Worker cannot resolve inputs. Update TUI and worker together; input preserved."),
    ...
    (
        "hint.worker_down",
        "Ctrl+R или /reconnect — переподключить движок",
        "Ctrl+R or /reconnect — reconnect the worker",
    ),
    (
        "hint.cancel_batch",
        "Esc — после файла · ещё Esc — прервать с подтверждением",
        "Esc stops after this file · Esc again asks to terminate",
    ),
    (
        "hint.cancel_llm",
        "Esc — отменить запрос · ещё Esc — прервать с подтверждением",
        "Esc cancels the request · Esc again asks to terminate",
    ),
    (
        "hint.add_files",
        "Перетащите файлы/папку или нажмите [Добавить]",
        "Drop files/folders or choose [Add]",
    ),
    (
        "hint.start",
    ...
    (
        "hint.run_llm",
        "F9 — результаты · L/F6 — выжимка/задачи через LLM",
        "F9 results · L/F6 summary or tasks through the LLM",
    ),
    (
        "hint.view_result",
        "F9 — сохранённые файлы · F2 — ответ LLM · F4 — журнал",
        "F9 saved files · F2 LLM answer · F4 log",
    ),
    ("queue.title", "Очередь", "Queue"),
    (
        "queue.empty",
        "Перетащите или вставьте файлы — они появятся здесь",
        "Drop or paste files — they will appear here",
    ),
    ("queue.col_no", "№", "#"),
    ...
    ("progress.overall", "Всего", "Overall"),
    ("progress.file", "Файл", "File"),
    ("progress.summary", "Готово {ok} · Ошибок {failed} · Не начато {unstarted} · Прервано {interrupted} · {time}", "Done {ok} · Failed {failed} · Unstarted {unstarted} · Interrupted {interrupted} · {time}"),
    ("results.title", "Результаты", "Results"),
    ("results.empty", "Сохранённых файлов пока нет", "No saved files yet"),
    ("results.path", "Полный путь · PgUp/PgDn", "Full path · PgUp/PgDn"),
    ("results.open", "Enter: Открыть", "Enter: Open"),
    ("results.folder", "O: Папка", "O: Folder"),
    ("results.close", "Esc: Закрыть", "Esc: Close"),
    ("results.opening", "Открываем…", "Opening…"),
    ("results.opened", "Передано системному просмотрщику", "Sent to the system viewer"),
    ...
    ("btn.cancel_llm", "Отменить запрос", "Cancel request"),
    ...
    (
        "help.key_esc",
        "закрыть панель; остановить (ещё раз — подтверждение); выход (×2)",
        "close panel; stop (again to confirm termination); quit (×2)",
    ),
    ("help.key_reconnect", "переподключить движок без повтора задания", "reconnect without restarting the job"),
    (
        "help.key_q",
        "выход вне режима ввода и обработки",
        "quit outside editing and processing",
    ),
    ...
    (
        "help.key_left_right",
        "ввод: курсор; вне ввода: очередь ↔ параметры",
        "editing: cursor; otherwise: queue ↔ parameters",
    ),
    (
        "help.key_enter",
    ...
    (
        "cmd.clear",
        "очистить очередь, сохранить результаты",
        "clear the queue, preserve results",
    ),
    (
        "cmd.llm-file",
    ...
    ("input.files", "Файлы", "Files"),
    ("input.command", "Команда", "Command"),
    ("input.argument", "Параметр", "Argument"),
    ("queue.add", "Добавить", "Add"),
    ("queue.undo", "Вернуть", "Undo"),
    ("help.key_insert", "открыть ввод файлов/папок", "open file/folder input"),
    ("help.key_undo", "вне ввода: вернуть удалённый файл", "outside editing: restore removed file"),
    ("help.key_edit", "ввод: начало/конец строки, удалить символ; Esc закрыть", "editing: start/end of line, delete a character; Esc closes"),
    ("help.key_clear_input", "очистить редактируемую строку", "clear the active input"),
    ("queue.actions", "Действия", "Actions"),
    ("queue.full_path", "Полный путь", "Full path"),
    ("queue.path_close", "↑↓ прокрутка · Esc/Enter закрыть", "↑↓ scroll · Esc/Enter close"),
    ("queue.run_pending", "Запустить ожидающие", "Run pending files"),
    ("queue.run_failed", "Повторить ошибки", "Retry failed files"),
    ("queue.run_selected", "Обработать выбранный", "Process selected file"),
    ("confirm.yes", "Да", "Yes"),
    ("confirm.no", "Нет", "No"),
    ("hint.adding_inputs", "Добавляем файлы; очистка отменит добавление", "Adding files; Clear cancels pending additions"),
    ("hint.batch_starting", "Запускаем обработку; повторный запуск заблокирован", "Starting processing; duplicate starts are blocked"),
    ("hint.queue_actions", "Enter / Действия — повтор, полный путь или отмена удаления", "Enter / Actions — retry, full path or undo removal"),
    ("queue.restored", "Удалённый элемент возвращён в очередь", "Removed item restored to the queue"),
    ("cmd.retry", "повторить неудачные файлы", "retry failed files"),
    ("cmd.run-selected", "обработать выбранный файл", "process the selected file"),
    ("cmd.undo", "вернуть удалённый элемент очереди", "restore the removed queue item"),
    ("cmd.add", "ввести пути файлов или папок", "enter file or folder paths"),
    ("cmd.retry-input", "повторить последнее неудачное добавление", "retry the last failed input"),
    ("status.queued", "Добавлено: {files}", "Queued {files}"),
    (
        "status.queued_report",
        "Добавлено: {added} · уже в очереди: {duplicates} · ошибок: {errors}",
        "Added: {added} · already queued: {duplicates} · errors: {errors}",
    ),
    (
