...
- **MCP stdio:** harness запускает `gigaam mcp`, общается через stdin/stdout и
  вызывает инструменты. Модель остаётся загруженной в процессе сервера.
...
Каждый harness обычно запускает **свой процесс и свою модель**. Одновременный
запуск шести клиентов не означает совместное использование одного ASR-процесса.
...
Успешный `--help` доказывает только импорт зависимостей. `server_status` должен
показывать `asr.loader_loaded=true`, `active_backend=mlx`, `error=null`.

Проверка через Python MCP SDK из того же venv; сначала прогрейте модель
headless-командой. Замените путь к WAV реальным локальным файлом:
